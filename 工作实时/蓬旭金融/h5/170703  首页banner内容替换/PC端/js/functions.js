...
function offset(obj){
	...
	for(var i=0;i<arr.length;i++){
		...
		result.left+=result.left+arr[i].offsetLeft+num1;
		result.top+=result.top+arr[i].offsetTop+num2;
	}
	return result;
}
...
function drag(obj,options,callback){
	new drags(obj,options,callback);
}
function drags(obj,options,callback){
	var options=options||{};
	this.obj=obj;
...
/*将ajax用函数进行包装,方便使用,传入的参数是一个对象该对象包括
	url---------传入的地址，必须传入
	data--------给另一个页面传输的数据，默认为空
	type--------数据传输方式，默认为get方式
	success------传入的函数，一般用于将取回的数据显示到本界面*/
function ajax(obj){
	if(!obj.url){
		return false;
	}
