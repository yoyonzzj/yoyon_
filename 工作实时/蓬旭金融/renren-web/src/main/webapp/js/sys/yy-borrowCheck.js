$(function () {
    $("#jqGrid").jqGrid({
        url: '/zyBorrowVerify/trialList',
        postData:{'page':1, 'limit':10,'sidx':'','order':'','phone':'','userStatus':'','startTime':'','endTime':''},
        datatype: "json",
        colModel: [		
            { label: '用户手机号码', name: 'phone', width:130}, 
			{ label: '用户真实姓名',name: 'userName', width:130}, 			
			{ label: '性别', name: 'usersex', width:64},
			{ label: '年龄', name: 'userage', width:64},
			{ label: '户籍地址', name: 'createTime', width:202},
			{ label: '借款金额', name: 'createTime', width:80},
			{ label: '借款期限', name: 'createTime', width:80},
			{ label: '申请时间', name: 'createTime', width:202}, 
			{ label: '借款次数', name: 'createTime', width:80},
			{ label: '逾期次数', name: 'createTime', width:80},
			{ label: 'id', name: 'id', classes: 'idaa', hidden:true, width:233 }, 
			{ label: '失败次数', name: 'createTime', width:80},
			{ label: '借款历史', name: 'personal', sortable:false,
				formatter: function(cellvalue, options, rowObject){
					var html = '<a href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-lg" style="color:#0000FF" onclick="borrowHistory('+"'"+rowObject.userId+"'"+')">查看</a>'
					return html;
				}, width:80
			},
			{ 
...
//借款历史信息弹窗封装
function borrowHistory(userId){
	$(".aaaa").css("display","none");
	$(".borrowHistory-form").css("display","block");
	$(".guanbi").css({"bottom":"10px","display":"block"});
	$("#quxiao").css("display","none");
	$.post('/zyBorrowVerify/queryUserLimit?userId='+userId,function(data){
		//获得响应数据
		var floatUserAltogetherLimit=parseFloat(data.result.userAltogetherLimit).toFixed(2);
		var floatQuotaUsed=parseFloat(data.result.quotaUsed).toFixed(2);
		var floatUserLastLimit=parseFloat(data.result.userLastLimit).toFixed(2);
		$("#history-userName").html("").append(function(){if(data.result.userName==undefined){return "---的历史记录"}else{return data.result.userName+"的历史记录"}});
		$("#history-userAltogetherLimit").html("").append(function(){if(floatUserAltogetherLimit==undefined){return "授信额度：---"}else{return "授信额度："+floatUserAltogetherLimit}});
		$("#history-quotaUsed").html("").append(function(){if(floatQuotaUsed==undefined){return "已用额度：---"}else if(floatQuotaUsed==""){return "已用额度：---"}else{return "已用额度："+floatQuotaUsed}});
		$("#history-userLastLimit").html("").append(function(){if(floatUserLastLimit==undefined){return "可用额度：---"}else if(floatUserLastLimit==""){return "可用额度：---"}else{return "可用额度："+floatUserLastLimit}});
	},'json');
	$.post('/zyBorrowVerify/queryBorrowList?userId='+userId,function(data){
		//获得响应数据
		...
		$(obj).each(function (index){
			...
			tr.append('<td>'+ index_order + '</td>' + '<td>'+ val.overdueDay + '</td>'+'<td>'+ val.borrowLimit + '</td>'+'<td>'+ val.borrowPeriods + '</td>'+'<td>'+ val.grantDate + '</td>'+'<td>'+ val.borrowStatus + '</td>');
			$("#data_tr").append(tr);
		});
	},'json');
}
...
function check(borrowVerifyId){
	...
	$.post('/zyBorrowVerify/getVerifyInfo?borrowVerifyId='+borrowVerifyId,function(data){
		//获得响应数据
		...
		$("#check-username").html("").append(function(){if(data.result.username==undefined){return "---"}else{return data.result.username}});
	  	...
	  	$("#yincang").html("").append(data.result.borrowVerifyId);
	},'json');
}

$("#tongguo").click(function(shenhemoney,borrowVerifyId){
	var shenhemoney=$("#optionFund").find("option:selected").text();
	var borrowVerifyId=$("#yincang").text();
	$.post('/zyBorrowVerify/firstVerify?borrowVerifyId='+borrowVerifyId+'&borrowVerifyStatus='+2,function(data){
		location.reload();
	},'json');
});
$("#butongguo").click(function(shenhemoney,borrowVerifyId){
	var shenhemoney=$("#optionFund").find("option:selected").text();
	var borrowVerifyId=$("#yincang").text();
	$.post('/zyBorrowVerify/firstVerify?borrowVerifyId='+borrowVerifyId+'&borrowVerifyStatus='+1,function(data){
		location.reload();
	},'json');
});
//查询按钮
$("#chaxun").click(function(){
	...
	var endTime=document.querySelector('#datetimeEnd').value;
	 $('#jqGrid').jqGrid('setGridParam',{  
	        datatype:'json',
	        page:1,
	        url: '/zyBorrowVerify/firstList',
	        postData:{'page':1, 'limit':10,'userStatus':userStatus,
	        	'startTime':startTime,'endTime':endTime,'sidx':'','order':'','phone':phone}
...
$("#refuse").click(function(){
	...
	var pass_number=ids.length;
	confirm('请确认是否批量拒绝已选中'+pass_number+'条记录？', function(){
		$.ajax({
		    url: "/zyuserverify/deleteBanner?bannerId="+ids,
		    success: function(r){
				if(r.code == 0){
					alert('操作成功', function(index){
						location.reload();
