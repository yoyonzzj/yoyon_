$(function () {
    $("#jqGrid").jqGrid({
        url: '/zyBorrowVerify/finalListNoCondition',
...
//					var html='<a href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-lg" onclick="identification('+"'"+rowObject.userId+"'"+')"">查看认证信息</a>';
			...
			{ 
				label: '初审结果', 
				name: 'firstBorrowStatus', sortable:false,
				formatter : function(cellvalue, options, rowObject){
            		if(rowObject.firstBorrowStatus == 1){
            			var html='<a href="javascript:void(0)" style="color:red;text-decoration:none;font-weight:bold">未通过</a>';
            		}else if(rowObject.firstBorrowStatus == 0){
			...
			{ 
				label: '复审结果', 
				name: 'secondBorrowStatus', sortable:false,
				formatter : function(cellvalue, options, rowObject){
            		if(rowObject.secondBorrowStatus == 1){
            			var html='<a href="javascript:void(0)" style="color:red;text-decoration:none;font-weight:bold">未通过</a>';
            		}else if(rowObject.secondBorrowStatus == 0){
            			var html='<a href="javascript:void(0)" style="color:#4F66FF;text-decoration:none;font-weight:bold">通过</a>';
            		}else{
            			var html='';
            		}
			...
			{ 
				...
				formatter: function(cellvalue, options, rowObject){
					if (rowObject.userStatus == 1) {
						var html = '<a href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-lg" onclick="check('+"'"+rowObject.borrowVerifyId+"'"+')">终审</a>'
					} else {
						var html = '<a href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-lg">---</a>'
					}
//					var html='<a href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-lg" onclick="check('+"'"+rowObject.borrowVerifyId+"'"+')"">审核</a>';
					return html
				}, width:70
			}
        ],
...
function check(borrowVerifyId){
	...
	$.post('/zyBorrowVerify/getVerifyInfo?borrowVerifyId='+borrowVerifyId,function(data){
		...
		$("#check-username").html("").append(function(){if(data.result.username==undefined){return "---"}else{return data.result.username}});
	  	...
	  	$("#check-firstBorrowStatus").html("").append(function(){if(data.result.firstBorrowStatus==undefined){return "---"}else if(data.result.firstBorrowStatus==1){return "未通过"}else if(data.result.firstBorrowStatus==0){return "通过"}else{return "错误"}});
	  	$("#check-secondBorrowStatus").html("").append(function(){if(data.result.secondBorrowStatus==undefined){return "---"}else if(data.result.secondBorrowStatus==1){return "未通过"}else if(data.result.secondBorrowStatus==0){return "通过"}else{return "错误"}});
...
$("#tongguo").click(function(shenhemoney,borrowVerifyId){
	...
	$.post('/zyBorrowVerify/finalVerify?borrowVerifyId='+borrowVerifyId+'&borrowVerifyStatus=6',function(data){
		location.reload();
	},'json');
});
$("#butongguo").click(function(shenhemoney,borrowVerifyId){
	var shenhemoney=$("#optionFund").find("option:selected").text();
	var borrowVerifyId=$("#yincang").text();
	$.post('/zyBorrowVerify/finalVerify?borrowVerifyId='+borrowVerifyId+'&borrowVerifyStatus=5',function(data){
		location.reload();
	},'json');
});
//查询按钮
