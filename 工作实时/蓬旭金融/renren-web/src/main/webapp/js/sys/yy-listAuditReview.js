$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [		
			...
			{ 
				label: '初审状态', 
				name: 'firstExamineStatus', sortable:false,
				formatter : function(cellvalue, options, rowObject){
            		if(rowObject.firstExamineStatus == 0){
            			var html='<a href="javascript:void(0)" style="color:#FF0000;text-decoration:none;font-weight:bold">通过</a>';
            		}else if(rowObject.firstExamineStatus == 1){
            			var html='<a href="javascript:void(0)" style="color:#008000;text-decoration:none;font-weight:bold">未通过</a>';
            		}else{
            			var html='';
            		}
            		return html;
			...
			{ 
				...
				formatter: function(cellvalue, options, rowObject){
					var html = '<a href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-lg" style="color:#0000FF" onclick="check('+"'"+rowObject.id+"'"+')">终审</a>'
					return html
				}, width:125
			}
        ],
	...
	methods: {
		...
		del1: function (event) {
			...
			confirm('请确认是否批量通过已选中'+pass_number+'条记录？', function(){
				$.ajax({
				    url: "/zyuserverify/batchSuccess?verifyId="+bannerIds,
				    success: function(r){
						if(r.code == 0){
		...
		del2: function (event) {
			...
			confirm('请确认是否批量拒绝已选中'+pass_number+'条记录？', function(){
				$.ajax({
				    url: "/zyuserverify/batchFail?verifyId="+bannerIds,
				    success: function(r){
						if(r.code == 0){
							alert('操作成功', function(index){
								location.reload();
...
function join(bannerId,Id){
	...
	$("#baocun").on("click",function(){
		...
		confirm('请确认是否将该用户添加到黑名单', function(){
			$.ajax({
			    url: '/zyuserverify/addBlack?userId='+bannerId+'&type='+type+'&reason='+reason+'&Id='+Id,
			    success: function(r){
					if(r.code == 0){
						alert('操作成功', function(index){
							location.reload();
...
function check(id){
	...
	$.post('/zyuserverify/userToExamine?verifyId='+id,function(data){
		...
		$("#check-verifyMoney").html("").append(function(){if(data.result.verifyMoney==undefined){return "---"}else{return data.result.verifyMoney}});
		$("#check-recommendcode").html("").append(function(){if(data.result.recommendcode==undefined){return "---"}else{return data.result.recommendcode}});
	  	$("#check-verifyUser").html("").append(function(){if(data.result.verifyUser==undefined){return "---"}else{return data.result.verifyUser}});
	  	$("#check-username").html("").append(function(){if(data.result.username==undefined){return "---"}else{return data.result.username}});
	  	$("#check-firstExamineStatus").html("").append(function(){if(data.result.firstExamineStatus==undefined){return "---"}else if(data.result.firstExamineStatus==0){return "通过"}else if(data.result.firstExamineStatus==1){return "未通过"}{return "错误"}});
	  	$("#yincang").html("").append(data.result.id);
	  	$("#yincang1").html("").append(data.result.userId);
	},'json');
}
...
$("#butongguo").click(function(shenhemoney,shenheid){
	...
	var userId=$("#yincang1").text();
	var phone=$("#check-phone").text();
	$.post('/zyuserverify/reviewExamine?verifyId='+shenheid+'&verifyStatus=5&money='+shenhemoney+'&userId='+userId+'&phone='+phone,function(data){
		location.reload();
	},'json');
});
//查询按钮
$("#chaxun").click(function(){
	...
	var firstExamineStatus=document.querySelector('#firstExamineStatus').value;
	if(firstExamineStatus=="通过"){
		firstExamineStatus=0;
	}else if(firstExamineStatus=="未通过"){
		firstExamineStatus=1;
	}else{
		firstExamineStatus="";
	}
	 $('#jqGrid').jqGrid('setGridParam',{  
	        ...
	        postData:{'page':1, 'limit':10,'username':username,'sidx':'','order':'',
	        	...
	        	'firstExamineStatus':firstExamineStatus,'ageStart':ageStart,'ageEnd':ageEnd}
