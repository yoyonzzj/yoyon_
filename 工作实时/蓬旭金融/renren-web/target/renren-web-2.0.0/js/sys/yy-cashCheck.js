$(function () {
    $("#jqGrid").jqGrid({
        url: '/zyuserwithdrawals/firstExamine',
        ...
        colModel: [		
			...
			{ label: '申请提现金额', name: 'applicationMoney', width:120}, 			
			{ label: '账户余额', name: 'accountBalance', width:100}, 			
			{ label: '可用余额', name: 'availableBalance', width:100}, 			
			{ label: '冻结金额', name: 'freezeMoney', width:100},	
			...
			{ 
				...
				name: 'createDate2', 
				formatter: function(cellvalue, options, rowObject){
					var html='<a href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-lg" onclick="check('+"'"+rowObject.withdrawalsId+"'"+')"">初审</a>';
					return html
				}, width:80
			}
        ],
...
$("#tongguo").click(function(shenhemoney,shenheid){
	...
	$.post('/zyuserwithdrawals/firstVerify?withdrawalsId='+shenheid+'&withdrawalsStatus='+2,function(data){
//		if(data.code==0){
//			alert("审核通过");
//		}else{
//			alert("信息填写有误");
...
$("#butongguo").click(function(shenhemoney,shenheid){
	var shenhemoney=$("#optionFund").find("option:selected").text();
	var shenheid=$("#yincang").text();
	$.post('/zyuserwithdrawals/firstVerify?withdrawalsId='+shenheid+'&withdrawalsStatus='+1,function(data){
//		if(data.code==0){
//			alert("审核不通过");
//		}else{
//			alert("信息填写有误");
