$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [		
			...
			{ label: '申请金额', name: 'verifyMoney', width:80}, 			
			{ label: '申请期限', name: 'borrowPeriods', width:80}, 			
			{ label: '申请时间', name: 'createTime', width:180}, 			
			{ label: '还款总额', name: 'refundLimit', width:80},	
			{ label: '还款期限', name: 'refundTime', width:180},
			...
			{ 
				...
				name: 'createDate2', 
				formatter: function(cellvalue, options, rowObject){
					var html='<a href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-lg" onclick="check('+"'"+rowObject.id+"'"+')"">终审</a>';
					return html
				}, width:65
			}
        ],
...
function check(id){
	...
	$.post('/zyuserverify/userToExamine?verifyId='+id,function(data){
		...
		$("#check-recommendcode").html("").append(function(){if(data.result.recommendcode==undefined){return "---"}else{return data.result.recommendcode}});
	  	...
	  	$("#optionFund option[value='"+data.result.verifyMoney+"']").attr("selected", true);
